...
/// <summary>
/// The order completed, so the units leave the building: both QuantityReserved and QuantityOnHand
/// fall. That is what separates a confirmation from a release, which only frees the hold.
/// </summary>
        ...
        await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (reservations.Count == 0)
            {
                _logger.LogInformation(
                    "No held reservations for order {OrderId}; nothing to confirm.", request.OrderId);
                return;
            }
