...
| [`ForgetProductTests`](../../server/tests/Ecommerce.Inventory.Tests/ForgetProductTests.cs) | A deleted product's stock rows go; its held reservations are released with them ("Product deleted"), settled ones stay, the sweeper and a later cancellation find nothing to act on (specs/024, 090). |
