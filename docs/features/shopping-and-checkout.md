...
20. **A hold expires.** A reservation still `Held` after `Inventory:ReservationTtlMinutes` (default 15) is returned to the shelf by `ReservationExpirySweeper` and marked `Expired`. *Why:* a saga that dies mid-flight would otherwise remove sellable stock permanently (specs/001 research D4).
...
23. **Removal is a decrement.** *Why:* anything the customer added while the order was in flight must survive.
...
| [`VariantLineTests`](../../server/tests/Ecommerce.Cart.Tests/VariantLineTests.cs) | Two shapes of one product are two lines; a pre-variant line is still addressed by its product id. |
...
- **No discounts.** `DiscountTotal` exists and a CHECK constraint holds it at 0 ([#108](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/108)).
...
- **The saga has no timeout of its own.** ([#123](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/123)) A hold expires after `Inventory:ReservationTtlMinutes` (15). If payment were to answer after that, the order would still settle to `Paid` while `ConfirmStockCommandHandler` finds no `Held` row and deducts nothing (`Inventory.Tests/SettlementTests.A_settlement_arriving_after_expiry_changes_nothing`). With the stub's immediate answer this does not happen in practice; with a real provider it could.
- **Checkout depends synchronously on Catalog, Cart and Identity.** Any one being down refuses orders with 503.
- **Cart removal matches lines by product id, not by variant id.** ([#122](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/122)) `CheckoutOutcomes.TryApplyAsync` looks up `cart.Lines.FirstOrDefault(l => l.ProductId == item.ProductId)`, and `OrderSubmittedConsumer` passes `OrderItemDto.ProductId`, while every other cart path uses `SellableId`. With two variants of the same product in one cart, the decrement can land on the wrong line. No test covers that case.
...
- **Customers receive no email** about their order; notices are in-app only ([#102](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/102)).
