...
**Stock availability.** Inventory owns stock. Every stock change in Inventory publishes `StockAvailabilityChangedEvent` with `IsAvailable` and the time it was observed. Catalog records the flag per variant, guarded by `AvailabilityObservedAt`. It then recomputes the product's flag as "any active variant available". Responses say `"InStock"` or `"OutOfStock"`, never a count. A real number comes from Inventory's public `GET /api/stock/{variantId}`.
...
**Search and sort.** `GetPaginatedAsync` matches the search term with `unaccent(lower(...)) LIKE` against the product's own name. It also matches the name translated into the requested language, and does a plain `LIKE` on the product SKU. Sorting is by name (the default-language `Name`), or by price in the **requested** currency. Products with no price in that currency sort last.
...
10. **After a rejection, the seller resubmits explicitly. After a seller edits an approved product, it goes back to review automatically.** `ProductReview.AfterSellerEditAsync` runs before the one save in six handlers: set or remove a product translation, upload or remove the product image, and upload or remove a variant image. Why: an edit to an approved product is a change no moderator has seen. A rejected product is still being worked on, so its seller says when it is ready (specs/045 D3). Prices, stock and new variants do not send a product back, and neither does anything staff edit. This was decided with the user.
...
15. **Search ignores diacritics on both sides and has no index.** `unaccent()` is not `IMMUTABLE`, so PostgreSQL will not index an expression on it. That makes search a sequential scan: invisible at 76 products, and the first thing to fix at 100,000 (specs/021 D5, [#113](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/113)).
...
- **Search scans every product.** There is no index behind `unaccent`. See [#113](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/113).
...
- **The per-variant availability guard is weaker than the product-level one.** ([#124](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/124)) `TryRecordVariantAvailabilityAsync` also requires the value to differ, so a newer same-value announcement does not advance `AvailabilityObservedAt`. An older opposite value that arrives after it can then still win. The product-level method's own comment warns against exactly this.
...
- **Some seller edits do not send a product back to review:** ([#126](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/126)) option translations, and adding a variant with new option text.
