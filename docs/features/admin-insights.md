...
**Catalog.** `POST /api/products/{id}/view` sends `RecordProductViewCommand`. The handler returns quietly unless the product exists and is on the shelf (`OnShelf`, specs/092), and the caller is not an administrator, not a moderator, and not the product's seller. Otherwise `ProductViewRepository.RecordAsync` runs one statement that claims the viewer's row in `product_viewers` and, only when that insert succeeds, increments `product_views."Views"` for (`ProductId`, today in the shop's time zone). The same statement deletes the product's viewers from earlier days (specs/086). `GET /api/products/insights/top-viewed` sums views per product between the dates of `from` and `to` (both days included), and joins `products` for the name.
...
7. **Views are incremented by the database.** `INSERT ... ON CONFLICT ("ProductId", "Day") DO UPDATE SET "Views" = product_views."Views" + 1`. Why: read-then-write loses views that arrive together (specs/047 D3). `ProductViewTests` fires twenty visitors at once and expects twenty.
...
9. **The view endpoint says nothing about the product.** It is anonymous and always answers 204, counted or not. Why: a different answer for an unlisted product would let anyone probe whether an id exists and is waiting for review.
   - **Once a day per viewer, and limited per client** (specs/086, #173). The viewer is the token's id for a signed-in person, whatever the body says; otherwise it is the random id the storefront keeps for the browser (`visitorId`, sent as `{ "viewer": ... }`). It is stored only as a SHA-256 in `product_viewers`, whose key is the claim. A call that names nobody (an older storefront, or storage blocked) still counts. The gateway's `views` policy allows 30 calls a minute per client IP, which bounds what a loop can add. *Why:* the endpoint is anonymous, and every call used to add a view, so a reload or a script inflated "most viewed" at will.
...
- **A visitor who clears their storage, or a script sending a new visitor id each time, is a new viewer.** The gateway's limit of 30 a minute per IP is what bounds them (specs/086).
...
| [086-product-view-limits](../../specs/086-product-view-limits/) | #178 | A view counts once a day per viewer (`product_viewers`, the storefront's visitor id), and the gateway's `views` policy limits the endpoint (#173). |
