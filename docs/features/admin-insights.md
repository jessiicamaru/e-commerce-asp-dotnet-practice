...
| System | Catalog increments the view count per product per UTC day in the database. |
...
- `RevenueByDayAsync` groups by `CreatedAt.Date` (UTC) and `Currency`, summing `TotalAmount` and counting orders.
...
| [`product_views`](../reference/data-model.md#product_views) | Catalog | One row per product per UTC day, primary key (`ProductId`, `Day`), with an index on `Day`. |
