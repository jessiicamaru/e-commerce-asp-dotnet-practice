...
7. **One review per customer per product, enforced by the database.** There is a unique index on `product_reviews` (`ProductId`, `CustomerId`), and a second `PUT` edits the first review. A CHECK constraint (`CK_product_reviews_rating`) keeps `Rating` between 1 and 5. The validator says the same in words, and limits `Body` to 2000 characters.
...
10. **Hidden, never deleted.** Why: a moderator can be wrong, and the author's words stay on record (specs/046 D4). A hidden review leaves the product page and the average. Staff can list the hidden ones, with the reason, and restore any of them.
...
- **Two simultaneous first reviews by the same customer of the same product.** ([#127](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/127)) The unique index keeps one, and the other request fails with an unmapped database error (500) rather than a 409 or an edit.
...
- **A seller can review their own product** ([#127](https://github.com/jessiicamaru/e-commerce-asp-dotnet-practice/issues/127)) if they bought and received it, because sellers also hold `Customer`.
