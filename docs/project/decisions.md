...
| 67 | A product view counts once a day per viewer - the token's id, else a random id the browser keeps, stored only as a hash - and the gateway limits the endpoint per client | The endpoint must stay anonymous and quiet; neither guard alone stops both an honest reload and a loop. A salted IP was rejected: behind the storefront every browser shares nginx's address unless trusted | [specs/086](../../specs/086-product-view-limits/) |
| 68 | An administrator sees each email's state and reason but never its data, and sends a failed one again with a guarded update - never a reset or confirmation link | The data of those two is a token; their link has expired by the time anybody looks, so the person asks for a new one | [specs/087](../../specs/087-email-delivery/) |
