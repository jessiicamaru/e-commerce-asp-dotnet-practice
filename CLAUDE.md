...
`vite.config.ts` and `tsconfig.app.json`, so generated files need no editing. ⚠️ **Three of them are
edited anyway** - `select`, `combobox` and `dropdown-menu`, for item padding and so a select no longer
opens on top of its own trigger - and `shadcn add --overwrite` would silently undo that; the client
