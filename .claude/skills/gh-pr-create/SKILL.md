...
Match on: a number in the branch name (`feat/42-foo` → #42), then on title/keyword
...
Use `Closes #N` when the PR fully resolves it, `Refs #N` when it only relates.
...
fails. Drop the author from the list first.
