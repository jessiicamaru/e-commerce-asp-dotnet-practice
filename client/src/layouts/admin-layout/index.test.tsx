...
describe('AdminLayout (specs/043)', () => {
  it('shows an administrator every page', async () => {
    ...
    for (const name of ['Orders to ship', 'Seller payouts', 'Users', 'Audit log']) {
  ...
  it('shows a moderator only the pages a moderator can use, and opens on Users', async () => {
    renderConsole(renderAsModerator)

    expect(await screen.findByText('the users page')).toBeInTheDocument()
